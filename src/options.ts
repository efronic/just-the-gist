const load = async () => {
  const { GEMINI_API_KEY, GEMINI_MODEL, DISABLE_TRANSCRIPTS, SHOW_TRANSCRIPT_PANEL } = await chrome.storage.sync.get({
    GEMINI_API_KEY: '',
...
const save = async () => {
  ...
  await chrome.storage.sync.set({ GEMINI_API_KEY: apiKey, GEMINI_MODEL: model, DISABLE_TRANSCRIPTS: disableTranscripts, SHOW_TRANSCRIPT_PANEL: showTranscriptPanel });
